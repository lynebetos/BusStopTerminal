	...
	throw (Exception &)
{
	...
	if ( !::HttpSendRequestW (
			...
			/*SAFEFREE (szPostedA) ;
			ThrowException (HTTPCLIENT_ERR_HTTPSENDREQUEST_FAILED, ::GetLastError ()) ;*/
		if (::GetLastError() == ERROR_INTERNET_INVALID_CA || ::GetLastError() == ERROR_INTERNET_SEC_CERT_CN_INVALID)   //add by sherrypan   处理需要证书的HTTPS
		{
			DWORD dwFlags;
			DWORD dwBuffLen = sizeof(dwFlags);
			::InternetQueryOptionW(hRequest, INTERNET_OPTION_SECURITY_FLAGS, (LPVOID)&dwFlags, &dwBuffLen);
			dwFlags |= SECURITY_SET_MASK;
			::InternetSetOptionW(hRequest, INTERNET_OPTION_SECURITY_FLAGS, &dwFlags, sizeof(dwFlags));
			if (!::HttpSendRequestW(hRequest, NULL, 0, (void *)szPostedA, cchPosted.Value()))
			{
				SAFEFREE (szPostedA) ;
				ThrowException (HTTPCLIENT_ERR_HTTPSENDREQUEST_FAILED, ::GetLastError ()) ;
			}
