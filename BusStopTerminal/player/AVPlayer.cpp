...
const size_t  MAX_AUDIOQ_SIZE  = 1024;  // 一帧数据的指针大约64B
const size_t  MAX_VIDEOQ_SIZE  = 1024;  // 一帧数据的指针大约64B
const size_t  MAX_PICTURE_SIZE = 3;     // 一帧数据的指针大约16B(字节)    设为128时,总内存为200多M; 设为3时,总内存为60M;
...
//    m_nHeight		    = 0;
    ...
    m_stcAudioSync.iAVSyncType        = AV_SYNC_AUDIO_MASTER;   // DEFAULT_AV_SYNC_TYPE;当只有音频时，不能呢个用视频时钟
...
{   
    int iFlags = SDL_HWSURFACE|SDL_ASYNCBLIT; // |SDL_NOFRAME;   // SDL_NOFRAME 无边框和标题栏
...
{     
    // 打开文件并读取文件的头部,检测是否音视频文件
    if(av_open_input_file(&m_pavFormatCtx, pszFilePath, NULL, 0, NULL)!=0)
    {
...
//    url_set_interrupt_cb(decode_interrupt_cb);
    ...
    if( OpenFile(pszPath) )
    {
        ...
        {            
			...
			dbPos += dbIncrement;
			StreamSeek((int64_t)(dbPos * AV_TIME_BASE), dbIncrement);
            ...
            {                  
                ...
                SetScale(m_psdlSurface_Screen->w / 2, m_psdlSurface_Screen->h / 2);         // 设置分辨率为w*h
                SDL_AddTimer(1, SDLEventRefresh, &m_stcRefreshTmp);                 // 只需触发一次FF_REFRESH_EVENT事件，之后会递归触发
            }   
...
//                    DisplayPicture(m_nWidth, m_nHeight, stcVPTmp.psdlBmp);  // 显示图片，其分辨率为m_nWidth*m_nHeight
                    DisplayPicture(m_iScreenWidth, m_iScreenHeight, stcVPTmp.psdlBmp);  // 显示图片，其分辨率为m_nWidth*m_nHeight
...
int thread_avpacket_push(void *argv)
{
    ...
    while(! pVideoPlayer->m_s_bIsQuit)
    {
        ...
        if(sizeVideo > MAX_AUDIOQ_SIZE || sizeAudio > MAX_VIDEOQ_SIZE) 
        {
            // ---------------------------------------------------------------------------------  
        ...
        if(av_read_frame(pVideoPlayer->m_pavFormatCtx, &avpacket) < 0)  // av_read_frame()读取一个包并把它保存到AVPacket结构体中
        {
            // 检测结构体是否读取文件错误
...
int audio_decode_frame(STC_AudioCallback *pstcAudioCallback, uint8_t *uszAudioBuf,const  int iAudioBufSize, double *pdbPTS) 
{
    ...
    {   
        ...
        {            
            //SDL_Delay(10);    // 用SDL_CondWait代替了,因为等10ms后不一定有数据,而且可能10ms太长，用SDL_CondWait就能等待一个精确的时间，并且不会重复询问是否有数据
...
void   DisplayPicture(int iWidth, int iHeight, SDL_Overlay *psdlBmp)
{
    SDL_Rect sdlRect;
...
double  CAVPlayer::GetPTS(AVPacket*  pavpacket, AVFrame  *pavFrame)
{
    ...
    {
        dbPTS = *(uint64_t *)pavFrame->opaque;
    } 
